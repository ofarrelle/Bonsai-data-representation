...
for ind_dim, num_dims in enumerate(num_dims_list):
    ...
    if DO_OTHER_TOOLS:
        ...
        if 'DTNE' in methods:
            # Perform DTNE.
            tool = 'DTNE'
            all_tool_files = [os.path.basename(filepath) for filepath in
                              Path(os.path.join(args.output_folder, 'intermediate_files')).glob('{}*.npy'.format(tool))]
            all_tool_files = [filepath for filepath in all_tool_files if 'pdists' not in filepath]
            if (not RECALCULATE) and len(all_tool_files):
                tool_projected = {}
                for tool_file in all_tool_files:
                    # n_comps = int(phate_file.split('phate_')[1].split('.npy')[0])
                    tool_projected['all'] = np.load(os.path.join(args.output_folder, 'intermediate_files', tool_file),
                                                    allow_pickle=False)
            else:
                tool_projected = {}
                tool_projected['all'] = fit_DTNE(delta_gc_true)
                np.save(os.path.join(args.output_folder, 'intermediate_files', '{}.npy'.format(tool)),
                        tool_projected['all'], allow_pickle=False)
            DTNE_projected = tool_projected.copy()
    ...
    if DO_OTHER_TOOLS:
        ...
        if 'DTNE' in methods:
            tool = 'DTNE'
            all_tool_dist_files = [os.path.basename(filepath) for filepath in
                                    Path(os.path.join(args.output_folder, 'intermediate_files')).glob('{}_pdists.npy'.format(tool))]
            if RECALCULATE or (not len(all_tool_dist_files)):
                for n_comps, tool_proj in DTNE_projected.items():
                    tool_dists = distance.pdist(tool_proj.T, metric='sqeuclidean') / 2
                    np.save(os.path.join(args.output_folder, 'intermediate_files', '{}_pdists.npy'.format(tool)),
                            tool_dists, allow_pickle=False)
                DTNE_dists = tool_dists
    ...
    for distfile in alldistfiles:
        ...
        datasets.append(
            Dataset(pdist_file=distfile, data_type=data_type, data_id=data_id,
    ...
    if num_dims == 2:
        ...
        ax3[4].scatter(DTNE_projected['all'][0, :], DTNE_projected['all'][1, :],
                       c=colors)
